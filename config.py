...
# ==========================================================
# Al-Borg lab/imaging booking - fixed hidden entities
# ==========================================================
#
# The Booking API has no concept of "book a test, no doctor needed" -
# create_new_booking always requires a doctor_id and a branch_id. So a
# fixed, real doctor record stands in for "whoever draws the sample",
# one per collection mode - its name is never shown to the patient.
# Home-collection bookings also all land on one fixed real branch
# record (not the patient's actual nearest branch - a human team goes
# to the patient's address regardless of which branch dispatches them).
#
# These three strings are the single source of truth other code reads
...
# working hours. Everything else about a branch (name, address, mobile)
# is live from the Booking API itself (see api.get_branches /
# tools._shape_branch) and must never be duplicated here - this file
# only supplies what the API cannot.
...
def _all_branches_geo() -> Dict[str, dict]:
    """branch_name (EXACTLY as returned by api.get_branches's own `name`
    field) -> {"latitude", "longitude", "working_hours"}, loaded once
    and cached - same pattern as `_all_client_configs` below.

    A row whose branch_name doesn't match any real branch the API
    returns is simply never looked up (see tools.find_nearest_branch,
    which joins the two by name) - it is not an error, just dead data
    until the CSV is corrected.
    """

    rows = _read_csv_rows(BRANCHES_GEO_CSV_FILENAME)
...
def load_branches_geo() -> Dict[str, dict]:
    """Public accessor for `_all_branches_geo` - callers outside this
    module go through here rather than touching the lru_cache directly."""

...
_CUES: Dict[str, List[Tuple[int, str]]] = {
    ...
    "booking": [
        ...
        # LAB/IMAGING VOCABULARY - this clinic books sample collection,
        # not doctor consultations, so "سحب عينة"/"تحليل"/"أشعة" ARE the
        # booking-intent words here (the generic كشف/دكتور/موعد cues
        # above don't fire for them at all - confirmed: a bare "سحب
        # عينة" or "عايز اعمل تحليل دم" scored nothing and fell through
        # to concierge, which still has the OLD doctor-booking prompt
        # with no idea about the in-lab/home-collection flow).
        (9, r"(?:سحب\s*عين[ةه]|عين[ةه]\s*دم|اعمل\s*تحليل|أعمل\s*تحليل|"
             r"احجز\s*تحليل|أحجز\s*تحليل|احجز\s*أشعة|احجز\s*اشعة|"
             r"عايز\s*(?:اعمل|أعمل)\s*(?:تحليل|تحاليل|اشعة|أشعة)|"
             r"عاوز\s*(?:اعمل|أعمل)\s*(?:تحليل|تحاليل|اشعة|أشعة))"),
        # Same intent, no عايز/عاوز lead-in ("محتاجة اعمل أشعة",
        # "هعمل تحليل بكرة") - the verb+noun pair alone is unambiguous
        # enough on its own for this lab-only clinic.
        (8, r"(?:اعمل|أعمل|هعمل|احتاج|محتاج[ةه]?)\s*(?:تحليل|تحاليل|اشعة|أشعة)"),
        (6, r"(?:^|\s)(?:تحليل|تحاليل|عينة|عينه|اشعة|أشعة)(?:\s|$|\?|؟)"),
        (9, r"\b(?:book|need|want)\b[^.\n]{0,25}\b(?:blood\s*test|lab\s*test|sample|scan|x-?ray)\b"),
    ...
    "faq": [
        ...
        # "أقرب فرع" / "nearest branch" - a real request to geocode the
        # patient's own address and rank branches by distance (see
        # tools.geocode_address / find_nearest_branch), owned by faq
        # alongside every other branch-info question above.
        (10, r"(?:اقرب|أقرب)\s*(?:فرع|معمل)"),
        (10, r"\bnearest\s+branch\b|\bclosest\s+branch\b"),
    ],
    CONCIERGE: [

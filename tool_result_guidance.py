...
from config import (
    ...
    LAB_IN_PLACE_DOCTOR_NAME,
    LAB_HOME_DOCTOR_NAME,
...
def find_available_doctors(
    ...
    # MAINTAINER NOTES - MOVED OUT OF THE DOCSTRING.
    # A tool's docstring IS its `description`, sent to the model on every
    # call by every agent bound to it. The notes below are for whoever
    # next edits this function - they are not instructions the model can
    # act on, so they live here rather than being re-billed every turn.
    # Nothing has been deleted.
    #
    # CONFIRMED REAL PRODUCTION FAILURE (`specialty_name`): a patient
    # picked "1" from a freshly shown specialty list, and the doctor found
    # for the id then used turned out to be scheduled under an unrelated,
    # more generic service - the same class of bug this parameter's
    # remembered-list resolution exists to prevent for every other list in
    # this project (doctors, branches, services, days).
    #
...
# ==========================================================
# Nearest-branch search (real geocoding, real branch data - no LLM
...
# every request - an unset/generic one risks the shared public instance
# rate-limiting or blocking this clinic's traffic entirely. Overridable
# via env so a deployment can point it at its own contact address.
...
def _haversine_km(lat1: float, lon1: float, lat2: float, lon2: float) -> float:
    ...
    Pure math - not a candidate for LLM estimation."""
...
def geocode_address(
    ...
    NEVER estimate, recall, or invent coordinates yourself for any
    address - always call this tool and use exactly the latitude/
    longitude it returns. This is step 1 of finding the nearest branch;
    follow it with `find_nearest_branch(latitude=..., longitude=...)`.
...
def find_nearest_branch(
    ...
    Combines two real sources, never guessed: the clinic's live branch
    list (name/address/phone straight from the Booking API, exactly
    like `list_branch_services`'s branch info) and this clinic's own
    lat/lng + working-hours data (the Booking API itself has no
    coordinates). A branch that has no lat/lng configured is skipped,
    not guessed at.
    ...
    if not ranked:
        logger.info(
            "find_nearest_branch: %d branch(es) returned by the API, none had geo "
            "data in branches_geo.csv", len(items),
        )
        return {"status": "not_found"}
    ...
    logger.info(
        "find_nearest_branch: lat=%s lon=%s -> nearest=%r (%.2f km)",
...
# ==========================================================
# Al-Borg lab/imaging booking - service catalogue search + hidden
# doctor resolution
# ==========================================================
...
def search_lab_services(
    ...
    catalogue's own entries are named in English (e.g. "عايزة اعمل
    تحليل دم" must surface real catalogue entries like "CBC", "WBC",
    "RBC" - never answer from the model's own medical knowledge, and
    never invent a test name that isn't actually in the catalogue).
    ...
    if not base_url:
        logger.warning(
            "search_lab_services called but no doctors_base_url is configured for client_id=%s",
            state.get("client_id"),
        )
        return {"status": "not_configured"}
    ...
    if not result["success"]:
        logger.error(
            "search_lab_services: get_services failed: status_code=%s error=%s",
            result.get("status_code"), result.get("error"),
        )
        return _api_error(result)
...
def select_sample_collection_mode(
    ...
    IN THE LAB or AT HOME. Call this ONCE the patient has answered that
    question, before any service/slot selection.
    ...
    if mode not in ("in_lab", "home"):
        return {"status": "invalid_mode"}

    base_url = _doctors_base_url(state)
    if not base_url:
        logger.warning(
            "select_sample_collection_mode called but no doctors_base_url is "
            "configured for client_id=%s", state.get("client_id"),
        )
        return {"status": "not_configured"}
    ...
    if not doctors_result["success"]:
        logger.error(
            "select_sample_collection_mode: get_doctors failed: status_code=%s error=%s",
            doctors_result.get("status_code"), doctors_result.get("error"),
        )
        return _api_error(doctors_result)
    ...
    if doctor_match is None:
        logger.error(
            "select_sample_collection_mode: fixed doctor %r (mode=%s) not found "
    ...
    # this flow is that name never reaches the patient. A neutral,
    ...
    logger.info(
        ...
        session_id, mode, session.get("doctor_id"), session.get("branch_id"),
    )

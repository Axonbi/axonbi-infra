...
_BOOKING_ENTRY_ASK_DIRECTIVE = (
    ...
    "and no collection mode yet. This is STEP NB1's opening rung.\n\n"
    "Your reply is ONE question offering the two collection modes:\n\n"
    ...
    "book.\n"
    "  - Do NOT ask which branch yet - that comes after the collection "
    "mode, and only for in-lab.\n"
    "  - Do NOT ask about symptoms, how they are feeling, or for how "
    "long. This is a booking request, not test guidance - no comfort "
    "tip, no clarifying health question here (that's the MEDICAL "
    "GUIDANCE flow's job, if they need it).\n"
    ...
    "again: \"في المعمل\"/\"في البيت\" (or any equivalent phrasing) -> "
    "call `select_sample_collection_mode` with that mode and continue; "
    "a test/symptom named instead -> use `search_lab_services` or the "
    "MEDICAL GUIDANCE flow as appropriate, then still ask the collection "
    "mode before confirming.\n\n"
)


_BOOKING_ENTRY_DONT_KNOW_DIRECTIVE = (
    "============================================================\n"
    "THEY SAID THEY DON'T KNOW - HELP THEM DECIDE, DON'T RE-ASK\n"
    "============================================================\n"
    "You asked whether they want the sample drawn in the lab or at "
    "home, and the patient said they don't know / aren't sure. This is "
    "a logistics choice, not a medical one - there is no specialty to "
    "work out here.\n\n"
    "Briefly explain the real difference in your own words (in-lab: "
    "they visit a branch, usually faster to get seen; home: a team "
    "comes to their address, more convenient if going out is hard) and "
    "ask them to pick based on that. Do NOT call `list_specialties` or "
    "any doctor-related tool - neither exists in this flow.\n\n"
)
...
_SYMPTOM_IN_BOOKING_DIRECTIVE = (
    ...
    "============================================================\n"
    "You asked whether they want it in-lab or at home, and the patient "
    "answered with a SYMPTOM/REASON instead of a mode. That is a normal "
    "answer, not a change of subject: most people describe what's wrong "
    "before they've thought about logistics.\n\n"
    "SO: call `search_lab_services` with what they described (never "
    "guess a test yourself), show the real matching test(s) found, "
    "briefly why each relates, and the required ⚕️ notice - then end "
    "with the SAME collection-mode question you already asked (in-lab "
    "or home), now framed around booking the test(s) just found. Chain "
    "the tool call and the reply in THIS turn.\n\n"
    "DO NOT run the full MEDICAL GUIDANCE comfort/follow-up sequence "
    "here - they already said they want to book, two messages ago:\n"
    "  - no extra clarifying question about how long, how severe, or "
    "what else they feel (unless `search_lab_services` genuinely found "
    "nothing and you need more to try again)\n"
    "  - no separate offer to \"book if they'd like\" - proceed straight "
    "to the collection-mode question\n\n"
    "Only name a test `search_lab_services` actually returned. If "
...
_BOOKING_ENTRY_MESSAGE = {
    ...
    "en": (
        ...
        "Would you like to have it done in the lab, or have someone "
        "collect the sample from your home?"
    ),
}
...
_SPECIALTY_CHOICE_CORRECTION_DIRECTIVE = (
    ...
    "DIFFERENT CASE - CHECK THIS FIRST: if the patient did not describe "
    "a symptom this turn but simply agreed (\"اه\"/\"شوفلي\") to YOUR "
    "OWN earlier offer, and that offer already named ONE SPECIFIC "
    "specialty (\"تحب أشوف لك دكاترة ثانيين في تخصص طب نفسي؟\"), there "
    "is no ambiguity to resolve at all - re-asking or broadening to the "
    "full catalogue breaks the promise you just made and forces them to "
    "repeat themselves. Call `find_available_doctors` for THAT NAMED "
    "specialty specifically and answer about it directly - including "
    "saying plainly that it has no available doctors right now, if "
    "that is what comes back, rather than pretending you have not "
    "already found that out.\n\n"
    "CONFIRMED REAL PRODUCTION FAILURE: the assistant offered \"تحب "
    "أشوف لك دكاترة ثانيين في تخصص طب نفسي\", the patient said \"اه "
    "شوفلي دكاتره\", and the reply printed all seven OTHER specialties "
    "and asked which one - forcing the patient to type \"طب نفسي\" "
    "again before finally being told (correctly) that it has no "
    "available doctors, something the system already knew one turn "
    "earlier.\n\n"
)
...
_SPECIALTY_CATALOGUE_CORRECTION_DIRECTIVE = (
    ...
    "DIFFERENT CASE - CHECK THIS FIRST: if there is no symptom this turn "
    "at all and the patient simply agreed to YOUR OWN earlier offer that "
    "already named ONE SPECIFIC specialty, printing the full catalogue "
    "is not a partial improvement over asking which one - it is the "
    "exact same mistake. Answer about the NAMED specialty directly (call "
    "`find_available_doctors` for it and say what comes back, including "
    "\"no doctors available\" if that is the honest answer) instead of "
    "handing them a list of everything else. See `_SPECIALTY_CHOICE_"
    "CORRECTION_DIRECTIVE`'s own confirmed failure for the exact "
    "production case this covers.\n\n"
)
